...
from common.constants import ERROR, DEFAULT_LISTEN_PORT, \
    MAX_CONNECTIONS, ACTION, TIME, PRESENCE, \
    RESPONSE, MESSAGE, MESSAGE_TEXT, TIMEOUT, SOURCE, EXIT
...
@log
def process_client_message(message, messages_list, client, clients):
    ...
    if ACTION in message and message[ACTION] == PRESENCE and TIME in message \
        ...
        return
    # Если сообщение о выходе, то отключаем клиента и удаляем его из списка клиентов
    elif ACTION in message and message[ACTION] == EXIT:
        clients.remove(client)
        client.close()
        SERVER_LOGGER.debug(f'Клиент {client} отключен от сервера')
        return
    # Если это информационное сообщение и оно правильно, то добавляем его в очередь
    elif ACTION in message and message[ACTION] == MESSAGE and \
            TIME in message and SOURCE in message and MESSAGE_TEXT in message:
...
def main():
    """Здесь запускается сервер, принимает подключения от клиентов, принимает от
    них сообщения, отправляет сообщения клиентам"""

    # Получаем параметры, с которыми вызывался скрипт
        ...
        if writing_clients:
            for client in writing_clients:
                try:
                    ...
                    process_client_message(message, messages, client, clients)
